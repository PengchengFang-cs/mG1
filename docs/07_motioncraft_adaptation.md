...
- 由预测的 root token 算 4 维局部根：`[0]` 偏航角速度（由相邻帧 root_rot_6d 的水平朝向对算 atan2(cross, dot) × fps）、`[1][2]` 相邻帧 root_trans 的 x/y 差 × fps、`[3]` root_trans 的 z（根高）。最后一有效帧复制前一帧。
- FP32 计算；用**单独的局部根均值方差**归一化（新增统计文件 `local_root_stats.npz`，从训练集同样的窗口采样统计）。
...
- **位置编码**：改为 ARDY 式带符号索引——`token_index = 真实帧偏移`，第一个生成帧为 0，历史为负（稠密 −16..−1，稀疏按抽到的真实偏移落在 −138..−17），未来 0..F−1。需要支持负索引的位置编码（RoPE 位置可直接传负值，或按 ARDY 写一张正负拼接的正弦表）。这是非均匀间隔能被正确理解的前提。
