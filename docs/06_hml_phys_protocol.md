...
### 2.1c 物理指标跨论文对照（2026-09-17，Jerk/Duration 口径已对齐，见 §2.1e）
Floating / Penetration / Foot-sliding 在关节位置上按米计算，不经过学习出来的评估器，因此跨论文可比。校验：MIND 报的物理真值 Floating 15.58 mm，我们自测 15.51 mm，差 0.1 mm，定义一致。

| 方法 | Floating mm ↓ | 来源 |
|---|---|---|
| **我们 mc_v1 250k** | **13.9** | 自测 |
| 我们 mc_v1 100k | 15.4 | 自测 |
| 物理真值（我们 / MIND 报） | 15.5 / 15.6 | 自测 / 论文 |
| 我们 mc_v1 25k | 17.0 | 自测 |
| MIND | 17.1 | 论文 |
| 物理真值（SCRIPT 报） | 17.5 | 论文 |
| UniPhys（我们跑 / MIND 报） | 17.6 / 20.5 | 自测 / 论文 |
| SCRIPT Stage II | 17.6 | 论文 |
| CLoSD | 19.3 | MIND 报 |
| PDP | 24.9 | MIND 报 |
| Kimodo++ | 33.7 | MIND 报 |

不可比或不报的：
- **Jerk**：MIND 的物理真值 2.6×10⁻³ mm/frame³，我们 5.00，相差约 1800 倍 → 归一化方式不同，只能在同一篇内部比排序。
- **Duration**：只有 SCRIPT 报（Stage I 97.67% / Stage II 98.08% / 真值 100%），定义（存活时长占比 vs 完成片段比例）与 rollout 长度均未写明，需见代码才能确认口径；我们 0.61–0.71。MIND 不报。
- **Penetration / Skating**：MIND 称"所有物理方法均可忽略"，不报；我们 0 与 0.019–0.038。
- 未测：推力鲁棒性（SCRIPT，0–800 N × 0.2 s，Fall@2s）、动作平滑与切换突变（ADAPT 的 Smooth/Trans）、关节限位越界、力矩能量。

### 2.1d 训练时长与"听文本 vs 动得干净"的权衡（mc_v1，测试集）
| 步数 | R@1 | Duration | Floating | Skating | Jerk |
|---|---|---|---|---|---|
| 25k | 0.254 | 0.612 | 17.0 | 0.038 | 5.85 |
| 50k | 0.230 | 0.696 | 16.6 | 0.031 | 5.57 |
| 100k | 0.221 | 0.708 | 15.4 | 0.025 | 4.52 |
| 250k | 0.183 | 0.704 | 13.9 | 0.019 | 3.21 |
训练越久，机器人侧全面变好（250k 全面优于物理真值），文本匹配单调变差 → 过拟合表现为"退回平均动作"：稳、平滑、不听文本。


### 2.1e Jerk 与 Duration 的口径对齐（2026-09-17）
**Jerk**。MIND 与 UniPhys 都不给公式；SCRIPT 给出定义：关节位置的三阶有限差分，**评测时省略 Δt³ 并乘 10³ 转毫米**，在原始仿真输出（24 个 SMPL 刚体、z 轴向上、控制频率 30 Hz）上算，L2 范数对关节和帧取平均。因此单位是 mm/frame³，随帧率三次方变化，报数必须注明帧率与关节数。

MIND 表里的数值比 SCRIPT 小 1000 倍（MIND 物理真值 2.7172×10⁻³ vs SCRIPT 2.941），是米没有换算成毫米，表头写的 mm 实际是 m；UniPhys 的 mm/s³ 表头同理不成立。**经验验证**：我们在自己的物理真值上按上述口径算得 **2.289 mm/frame³ @30 Hz**，与 SCRIPT 的 2.941、MIND 的 2.7172（×1000 后）同一量级且相差 20% 到 28%，而按我们原来的 20 fps / 22 关节口径是 5.27，正好是帧率三次方的差别。剩下的 20% 来自跟踪器不同（我们用 PULSE，两篇用 PHC；PULSE 动作更平滑）。

| 方法 | Jerk mm/frame³ @30 Hz | 来源 |
|---|---|---|
| SCRIPT Stage II / I | 1.706 / 1.723 | 论文 |
| 物理真值（我们，PULSE） | 2.289 | 自测 |
| UniPhys（我们跑） | 2.533 | 自测 |
| MIND | 2.597 | 论文 ×1000 |
| 物理真值（MIND 报，PHC） | 2.717 | 论文 ×1000 |
| CLoSD（SCRIPT 报） | 2.767 | 论文 |
| 物理真值（SCRIPT 报，PHC） | 2.941 | 论文 |
| **我们 mc_v1 25k** | **3.027** | 自测 |
| PDP（SCRIPT 报） | 3.047 | 论文 |
| UniPhys（MIND 报） | 3.605 | 论文 ×1000 |
| PDP（MIND 报） | 6.548 | 论文 ×1000 |
| Kimodo++（MIND 报） | 10.271 | 论文 ×1000 |
| CLoSD（MIND 报） | 12.002 | 论文 ×1000 |
注意 MIND 与 SCRIPT 对同一基线给出的 Jerk 相差很大（CLoSD 12.0 vs 2.8，UniPhys 3.6 vs 2.0），说明该指标对 rollout 长度、是否含摔倒片段等协议细节高度敏感；我们自跑的 UniPhys 2.533 落在两者之间。摔倒片段会显著抬高 Jerk（我们 2.5→3.2，我们模型 3.0→4.2）。

**Duration**。SCRIPT 的定义是帧加权：所有 rollout 的有效仿真帧数之和除以参考帧数之和，摔倒判据为根部高度 < 0.15 m（PHC 默认）。不是"未摔片段比例"，也不是逐片段平均。按此口径重算：

| 口径 | UniPhys（我们跑） | 我们 mc_v1 25k | SCRIPT II |
|---|---|---|---|
| **Duration（帧加权，SCRIPT 口径）** | **0.871** | **0.818** | 0.981 |
| 未摔片段比例 | 0.711 | 0.612 | — |
| 逐片段存活占比平均 | 0.897 | 0.853 | — |
我们的摔倒判据是环境的终止条件（非脚部刚体触地且高度 < 0.15 m），比纯根高判据略早触发，因此我们的 Duration 是下界。

### 2.1f 原始仿真口径的物理指标（24 刚体、30 Hz、真实地面，与论文一致）
| 行 | Floating mm | Penetration | Skating mm | Jerk | Duration |
|---|---|---|---|---|---|
| 物理真值（我们） | 15.49 | 0 | 1.37 | 2.289 | 1.000 |
| UniPhys（我们跑） | 17.60 | 0 | 2.52 | 2.533 | 0.871 |
| 我们 mc_v1 25k | 17.19 | 0 | 2.34 | 3.027 | 0.818 |
（此前 §2.1c 表中"我们 250k 13.9 mm 最低"是 20 fps / 22 关节口径下的数，口径不同，以本表为准；250k 与 50k/100k 的 rollout 已删，如需该行要重跑。）
